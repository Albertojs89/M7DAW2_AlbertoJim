<?php
session_start();
$_SESSION['libreria'] = [
    [
        "Titulo" => "El Ritmo de la Guerra",
        "Autor" => "Brandon Sanderson",
        "Descripcion" => "Cuarta entrega de la saga de 'El Archivo de las Tormentas', una epopeya de fantasía épica con un desarrollo de personajes y un mundo increíblemente detallado.",
        "Imagen" => "https://m.media-amazon.com/images/I/91Nb4w7arrL.jpg",
        "id"=>"0"
        
    ],
    [
        "Titulo" => "El nombre del viento",
        "Autor" => "Patrick Ruffus",
        "Descripcion" => "Una historia cautivadora que combina misterio, fantasía.",
        "Imagen" => "https://m.media-amazon.com/images/I/91PjnllfsxL._AC_UF894,1000_QL80_.jpg",
        "id"=>"1"
        
    ],
    [
        "Titulo" => "La Chica del Tren",
        "Autor" => "Paula Hawkins",
        "Descripcion" => "Un thriller psicológico sobre la obsesión y la percepción de la realidad a través de los ojos de una mujer que lo ha perdido todo.",
        "Imagen" => "https://www.planetadelibros.com/usuaris/libros/fotos/197/original/la-chica-del-tren_9788408141471_contra.jpg",
        "id"=>"2"
        
    ],
    [
        "Titulo" => "Canción de Hielo y Fuego",
        "Autor" => "George R.R.Martín",
        "Descripcion" => "Saga fantasía ganadora de multiples premios en la serie de TV",
        "Imagen" => "https://m.media-amazon.com/images/I/71Ks3ve42xL._AC_UF894,1000_QL80_.jpg",
        "id"=>"3"
        
    ],
    [
        "Titulo" => "El Instituto",
        "Autor" => "Stephen King",
        "Descripcion" => "Un oscuro y atrapante thriller de ciencia ficción sobre un grupo de niños con habilidades especiales que son prisioneros en una misteriosa institución.",
        "Imagen" => "https://m.media-amazon.com/images/I/81s4yvUwSqL.jpg",
        "id"=>"4"
        
    ],
    [
        "Titulo" => "Proyecto Hail Mary",
        "Autor" => "Andy Weir",
        "Descripcion" => "Una novela de ciencia ficción que narra la misión de un astronauta solitario en una misión desesperada para salvar a la humanidad.",
        "Imagen" => "https://imagessl6.casadellibro.com/a/l/s5/16/9788418037016.webp",
        "id"=>"5"
        ,
    ]
];
?>
